...
func TestRemoveApp(t *testing.T) {
	...
	t.Run("domain__existing_app__removes_workspace_entry", func(t *testing.T) {
		...
		if err := workspace.WriteWorkspaceConfig(fs, workspace.WorkspaceConfig{
			Apps: []workspace.WorkspaceApp{
				workspace.MakeApp("alpha", nil),
				workspace.MakeApp("beta", nil),
			},
		}); err != nil {
			t.Fatalf("write workspace config: %v", err)
		}
...
func TestWireServiceToCompose(t *testing.T) {
	t.Run("domain__workspace_and_empty_compose__writes_service_entries", func(t *testing.T) {
		...
		service, err := compose.ReadComposeService(fs, composePath, "api")
		if err != nil {
		...
		devService, err := compose.ReadComposeService(fs, devComposePath, "api")
		if err != nil {
			t.Fatalf("read compose service: %v", err)
		}
	...
	t.Run("complement__service_already_exists__returns_error", func(t *testing.T) {
		...
		composePath := filepath.Join(appPath, "docker-compose.yml")
		if err := compose.WriteCompose(fs, composePath, map[string]any{
...
func TestAddServiceToCompose(t *testing.T) {
	t.Run("domain__empty_compose__adds_service", func(t *testing.T) {
		...
		if err := compose.AddServiceToCompose(fs, composePath, "api", map[string]any{
			"image": "alpha_api",
		}); err != nil {
...
func TestNextServicePort(t *testing.T) {
	t.Run("domain__existing_ports__returns_next_port", func(t *testing.T) {
		...
		port, err := services.NextServicePort(fs, "alpha")
		if err != nil {
	...
	t.Run("chaos__invalid_workspace_json__returns_error", func(t *testing.T) {
		...
		if _, err := services.NextServicePort(fs, "alpha"); err == nil {
			t.Fatalf("expected error")
		}
	})
}
