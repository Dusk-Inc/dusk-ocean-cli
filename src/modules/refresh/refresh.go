...
func RunRefresh(cmd *cobra.Command, fs afero.Fs, root string, config workspace.WorkspaceConfig) error {
	graph, index, err := deps.BuildWorkspaceGraph(config)
	if err != nil {
		return err
	}
	order, err := deps.SortDependencyGraph(graph)
	if err != nil {
		return err
	}
	if len(order) == 0 {
	...
	for _, key := range order {
		...
		label, path, _, err := deps.NodeBuildInfo(root, node)
		if err != nil {
			return err
		}
	...
	for _, key := range order {
		...
		label, path, hashPath, err := deps.NodeCheckInfo(root, node)
		if err != nil {
			return err
		}
		if err := workspace.RunCheck(cmd, label, path, hashPath, nil, root); err != nil {
			return err
		}
	}
...
func runInstall(cmd *cobra.Command, fs afero.Fs, label string, targetPath string) error {
	installCmd, err := workspace.ReadRepoCommand(fs, targetPath, "install")
	if err != nil {
		return err
	}
