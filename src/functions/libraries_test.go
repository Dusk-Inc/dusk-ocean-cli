...
func TestMakeAppLibNode(t *testing.T) {
	t.Run("domain__registered_library__returns_node", func(t *testing.T) {
		config := MakeConfig(
			nil,
			[]WorkspaceApp{
				MakeApp("alpha", []WorkspaceLibrary{
					MakeLibrary("lib-a", "dep-1"),
				}),
			},
			nil,
		)
		...
		assertAppLibNode(t, node, "alpha", "lib-a", []WorkspaceDep{
	...
	t.Run("boundary__empty_deps__returns_node_with_empty_deps", func(t *testing.T) {
		...
		assertAppLibNode(t, node, "beta", "lib-b", []WorkspaceDep{})
		if len(node.Deps) != 0 {
	...
	t.Run("complement__missing_library__returns_error", func(t *testing.T) {
		config := MakeConfig(
			nil,
			[]WorkspaceApp{
				MakeApp("gamma", []WorkspaceLibrary{
					MakeLibrary("lib-c"),
				}),
			},
			nil,
...
func TestMakeGlobalLibNode(t *testing.T) {
	t.Run("domain__registered_library__returns_node", func(t *testing.T) {
		...
		assertGlobalLibNode(t, node, "lib-a", []WorkspaceDep{
			{Lib: "dep-1", From: "global"},
		})
	})
	...
	t.Run("boundary__empty_deps__returns_node_with_empty_deps", func(t *testing.T) {
		...
		assertGlobalLibNode(t, node, "lib-b", []WorkspaceDep{})
		if len(node.Deps) != 0 {
			t.Fatalf("expected no deps, got %d", len(node.Deps))
		}
	})
...
func TestCollectLibraryDependents(t *testing.T) {
	t.Run("domain__global_library_dependents__returns_targets", func(t *testing.T) {
		...
		expected := map[string]TargetKind{
			"/root/repos/apps/alpha/services/api": TargetService,
			"/root/repos/apps/alpha/libs/lib-app": TargetAppLib,
			"/root/repos/libs/lib-consumer":       TargetGlobalLib,
			"/root/repos/projects/proj":           TargetProject,
		}
...
func assertAppLibNode(t *testing.T, node Node, appName string, name string, depsList []WorkspaceDep) {
	t.Helper()
	if node.Kind != NodeAppLib {
		t.Fatalf("expected kind %s, got %s", NodeAppLib, node.Kind)
	}
	if node.App != appName {
...
func assertGlobalLibNode(t *testing.T, node Node, name string, depsList []WorkspaceDep) {
	t.Helper()
	if node.Kind != NodeGlobalLib {
		t.Fatalf("expected kind %s, got %s", NodeGlobalLib, node.Kind)
	}
	if node.Name != name {
		t.Fatalf("expected name %s, got %s", name, node.Name)
	}
