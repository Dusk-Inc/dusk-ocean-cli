					...
					Services: []WorkspaceService{
						{
							Name:       "api",
							Port:       "3000",
							Image:      WorkspaceImage{Name: "custom-image", Tag: "old"},
...
func TestFormatImageReference(t *testing.T) {
	t.Run("domain__name_tag_registry__formats_full_reference", func(t *testing.T) {
		ref := formatImageReference(WorkspaceImage{
			Name: "image",
			Tag:  "v1",
		})
		if ref != "image:v1" {
...
func writeWorkspaceConfig(t *testing.T, fs afero.Fs, config WorkspaceConfig) {
	t.Helper()
...
func readWorkspaceConfig(t *testing.T, fs afero.Fs) WorkspaceConfig {
	t.Helper()
...
func findService(t *testing.T, app WorkspaceApp, name string) WorkspaceService {
	t.Helper()
